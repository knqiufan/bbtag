#!/usr/bin/env python3
"""Render DeepSeek account balance for 2.13-inch tags.
...
示例:
    ...
    uv run examples/push_deepseek_balance.py --device EDP-F3F4F5F6
...
DEFAULT_OUTPUT = "DeepSeek-balance-2.13inch.png"
...
def render_account_image(
    ...
    *,
    ...
    cash_text = f"CASH {format_money(account.cash_total, symbol=symbol)}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="把 DeepSeek 账户余额画成 2.13 寸电子价签样式并推送。",
    )
    ...
    parser.add_argument("--input-json", type=Path, help="本地 balance JSON，跳过网络")
    parser.add_argument("--api-key", help="最高优先级，直接作为 Bearer Token")
    parser.add_argument(
        "--settings-path",
        type=Path,
        help="额外 Claude settings.json（合并 env.ANTHROPIC_AUTH_TOKEN）",
    )
    parser.add_argument(
        "--currency",
